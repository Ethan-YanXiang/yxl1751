...
def get_clusters_from_db():
    clusters = [
        (cluster.id, list(map(float, cluster.cluster_center.split(","))))
...
def delete_old_news():
    ...
    affected_clusters = {
        article.cluster_id for article in articles_to_delete if article.cluster_id
    }
